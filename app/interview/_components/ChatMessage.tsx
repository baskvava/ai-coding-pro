...
function ChatMarkDownRenderer({ message }: { message: Message }) {
  return (
    <ReactMarkdown
...
function MessageBubble({ isAi, message }: { isAi: boolean; message: Message }) {
  return (
    <div
      className={`
          ...
          `}
    >
      <ChatMarkDownRenderer message={message} />
      {/* Timestamp */}
      <div
...
function Avatar({ isAi }: { isAi: boolean }) {
  return (
    <div
      className={`
            w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs text-white font-bold shadow-sm
            ${
              isAi
...
export const ChatMessage = ({ message }: { message: Message }) => {
  ...
  return (
    ...
    >
      ...
      >
        <Avatar isAi={isAi} />
        <MessageBubble isAi={isAi} message={message} />
      </div>
    </div>
  );
};
